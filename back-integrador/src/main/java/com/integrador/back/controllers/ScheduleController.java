...
public class ScheduleController {
    ...
    public ResponseEntity<?> getSchedulesAll(@RequestParam(defaultValue = "0") int page,
                                             ...
                                             @RequestParam(required = false) LocalDateTime today,
    ...
    @PostMapping()
    public ResponseEntity<?> saveSchedule(@Valid @RequestBody ScheduleCreateRequest scheduleCreateRequest, BindingResult bindingResult) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(scheduleService.saveSchedule(scheduleCreateRequest).get());
